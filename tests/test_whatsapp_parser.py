...
class TestFracoes:
    def test_1bpchq_11_fracoes(self):
        fracoes = parse_fracoes(TEXTO_1BPCHQ)
        assert len(fracoes) == 11

    def test_2bpchq_1_fracao(self):
        fracoes = parse_fracoes(TEXTO_2BPCHQ)
        assert len(fracoes) == 1

    def test_3bpchq_3_fracoes(self):
        fracoes = parse_fracoes(TEXTO_3BPCHQ)
        assert len(fracoes) == 3

    def test_4bpchq_4_fracoes(self):
        fracoes = parse_fracoes(TEXTO_4BPCHQ)
        assert len(fracoes) == 4

    def test_5bpchq_4_fracoes(self):
        fracoes = parse_fracoes(TEXTO_5BPCHQ)
        assert len(fracoes) == 4

    def test_6bpchq_3_fracoes(self):
        fracoes = parse_fracoes(TEXTO_6BPCHQ)
        assert len(fracoes) == 3

    def test_4rpmon_10_fracoes(self):
        fracoes = parse_fracoes(TEXTO_4RPMON)
        assert len(fracoes) == 10


class TestCabecalho:
    def test_1bpchq_cabecalho(self):
        cab, avisos = parse_cabecalho(TEXTO_1BPCHQ)
        assert cab["unidade"] == "1 BPChq"
        assert cab["efetivo_total"] == 154
        assert cab["vtrs"] == 25
        assert cab["motos"] == 12
        assert cab["armas_ace"] == 25
        assert cab["animais"] == 3

    def test_2bpchq_cabecalho(self):
        cab, avisos = parse_cabecalho(TEXTO_2BPCHQ)
        assert cab["unidade"] == "2 BPChq"
        assert cab["efetivo_total"] == 15

    def test_4rpmon_cabecalho(self):
        cab, avisos = parse_cabecalho(TEXTO_4RPMON)
        assert cab["unidade"] == "4 RPMon"
        assert cab["efetivo_total"] == 46
        assert cab["animais"] == 16

    def test_6bpchq_unidade(self):
        cab, avisos = parse_cabecalho(TEXTO_6BPCHQ)
        assert cab["unidade"] == "6 BPChq"


class TestHorarios:
    def test_horario_24hs(self):
        fracoes = parse_fracoes(TEXTO_5BPCHQ)
        germano = [f for f in fracoes if "Germano" in f["comandante"]]
        assert len(germano) == 1
        assert germano[0]["horario_inicio"] == "00:00"
        assert germano[0]["horario_fim"] == "23:59"

    def test_horario_retorno(self):
        fracoes = parse_fracoes(TEXTO_5BPCHQ)
        marlon = [f for f in fracoes if "Marlon" in f["comandante"]]
        assert len(marlon) == 1
        assert marlon[0]["horario_inicio"] == "Retorno"

    def test_horario_qtl(self):
        fracoes = parse_fracoes(TEXTO_4RPMON)
        souto = [f for f in fracoes if "SOUTO" in f["comandante"]]
        assert len(souto) == 1
